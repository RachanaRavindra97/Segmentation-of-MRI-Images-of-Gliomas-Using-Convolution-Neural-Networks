...
def parse_inputs():
    ...
    parser.add_argument('-r', '--root-path', dest='root_path', default=r'E:\UG Schooling\8th Sem\BRATS2018\HGG')
    parser.add_argument('-sp', '--save-path', dest='save_path', default='dense48_correction')
    parser.add_argument('-lp', '--load-path', dest='load_path', default='dense48_correction')
    ...
    parser.add_argument('-ws', '--width-size', dest='wsize', type=int, default=40)
    parser.add_argument('-hs', '--height-size', dest='hsize', type=int, default=40)
    parser.add_argument('-cs', '--channel-size', dest='csize', type=int, default=40)
    ...
    parser.add_argument('-e', '--num-epochs', dest='num_epochs', type=int, default=4)
    parser.add_argument('-c', '--continue-training', dest='continue_training', type=bool, default=False)
    parser.add_argument('-mn', '--model_name', dest='model_name', type=str, default='dense48')
...
def segmentation_loss(y_true, y_pred, n_classes):
    ...
    return tf.reduce_mean(tf.nn.softmax_cross_entropy_with_logits_v2(labels=y_true,
                                                                  logits=y_pred))
    ...
    while 1:
        for file in all_files:
            if correction:
                ...
                #gt = load_nii(os.path.join(path, file, file + '_seg.nii.gz')).get_data()
            #scans = [flair[0], t1[0], t1ce[0], t2[0], gt[0]]
...
def train():
    ...
    with tf.control_dependencies(update_ops):
        optimizer = tf.train.MomentumOptimizer(learning_rate=5e-4,momentum=0.9).minimize(loss)

    saver = tf.train.Saver(max_to_keep=15)
    data_gen_train = vox_generator(all_files=files, n_pos=155, n_neg=155,correction = options['correction'])
...
if __name__ == '__main__':
    #print('Number of trainable parameters are:', train().count_params())
    train()